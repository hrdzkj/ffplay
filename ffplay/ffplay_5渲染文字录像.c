...
typedef struct FFPlayer {
	...
	AVFrame *lastKeyFrame;
	// pthread_mutex_t record_mutex;       // 锁
	int is_record;                      // 是否在录制             
	int record_error;
	int is_first;                       // 第一帧数据 ---改为是否写关键帧
...
//查找媒体类型的流索引
static int getMediaTypeStreamIndex(FFPlayer *ffp, enum AVMediaType mediaType)
{
	int ret = -1;
	AVFormatContext *ifmt_ctx = ffp->is->ic;
	for (int i = 0; i < ifmt_ctx->nb_streams; i++) {
		AVStream *in_stream = ifmt_ctx->streams[i];
		AVCodecContext *codecCtx = in_stream->codec;
		if (codecCtx->codec_type == mediaType) {
			ret = i;
			break;
		}
	}
...
static void encode(FFPlayer *ffp,AVCodecContext *enc_ctx, AVFrame *frame)
{
	...
	while (ret >= 0) {
		...
		if (enc_ctx->coded_frame->key_frame && enc_ctx->coded_frame->pict_type == AV_PICTURE_TYPE_I) {
			ffp->is_first = 1;
		}

		//av_packet_rescale_ts(pkt, ffp->is->ic->streams[stream_index]->codec->time_base,stream_ctx[stream_index].enc_ctx->time_base);
...
int ffp_record_file(FFPlayer *ffp, AVPacket *packet)
{
	...
	if (0 == av_packet_ref(pkt, packet)) {
		...
		*/
		...
		pkt->duration = packet->duration;//av_rescale_q(pkt->duration, in_stream->time_base, out_stream->time_base);
		pkt->pos = -1;
...
int ffp_start_record(FFPlayer *ffp, const char *file_name)
{
	...
	for (int i = 0; i < is->ic->nb_streams; i++) {
		...
		if (dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO) {
			...
			//导致录像很大
			//enc_ctx->bit_rate = 40000;// dec_ctx->bit_rate;;
			enc_ctx->framerate = (AVRational) { 1, 30 }; //dec_ctx->framerate;
		...
		/*将输入视频/音频的参数拷贝至输出视频/音频的AVCodecContext结构体
		 avcodec_parameters_from_context(out_stream->codecpar, enc_ctx)不成功，可能是ffmpeg版本的bug。
		 avcodec_copy_context(out_stream->codec, in_stream->codec)  成功，已经弃用
		 avcodec_parameters_to_context(out_stream->codec, in_stream->codecpar)成功， 
		 https://blog.csdn.net/luotuo44/article/details/54981809 解释用法
		*/
...
static int deep_copy_frame(AVFrame *dst,AVFrame *src)
{
	if (dst && src) {
		...
		dst->pkt_pts = src->pkt_pts;//设置这些也无效
...
static int stream_component_open(FFPlayer *ffp, int stream_index)
{
	...
	case AVMEDIA_TYPE_VIDEO:
		...
		if ((ret = decoder_start(&is->viddec, video_thread, ffp)) < 0)
			goto out;
...
static void stream_cycle_channel(FFPlayer *ffp, int codec_type)
{
	VideoState *is = ffp->is;
	AVFormatContext *ic = is->ic;
...
int main(int argc, char **argv) {
	...
	"800",
	"-vf",
	"drawtext=fontfile=arial.ttf:fontsize=40:text='hello world':x=100:y=100",
	"test.mp4",
	};
	oldEntrence(8, &paras);
