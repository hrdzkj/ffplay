...
//录像 begin // frame 拷贝
/*
// http://ffmpeg.org/doxygen/3.4/transcoding_8c-example.html
//https://www.cnblogs.com/snow-boy/p/8259591.html
AVFormatContext *format_context_=NULL;
AVCodecContext *codec_context_ = NULL;
AVStream *video_stream_ = NULL;
static inline int StartReord(char *fileName)
{
	avformat_alloc_output_context2(&format_context_, NULL, NULL, fileName);
	if (format_context_ == NULL) {
		avformat_alloc_output_context2(&format_context_, NULL, "mpeg", fileName);
	}
	...
	int ret = avcodec_open2(codec_context_, codec, NULL);
	if (ret != 0) {
		goto end;
	}
...
//static FFPlayer *ffplay;

static int open_output_file(const char *filename, FFPlayer *ffp)
{
	AVStream *out_stream;
	AVStream *in_stream;
...
static int encode_write_frame(FFPlayer *ffp, AVFrame *filt_frame, unsigned int stream_index, int *got_frame) {
	...
	int(*enc_func)(AVCodecContext *, AVPacket *, const AVFrame *, int *) =
		(is->ic->streams[stream_index]->codecpar->codec_type ==
	...
	VideoState *is = ffp->is;
		...
		if (0 == av_packet_ref(pkt, packet)) {
			...
			if ((ret = av_interleaved_write_frame(ffp->m_ofmt_ctx, pkt)) < 0) { // todo 出现负数导致写包错误
				av_log(NULL, AV_LOG_ERROR, "liuyi----> faild: av_interleaved_write_frame");
			}
			else {
				av_log(NULL, AV_LOG_DEBUG, "liuyi----> ok : av_interleaved_write_frame");
			}
...
static int flush_encoder(VideoState *is,unsigned int stream_index)
{
	int ret;
	int got_frame;
	...
	while (1) {
		...
		ret = encode_write_frame(is,NULL, stream_index, &got_frame);
		if (ret < 0)
			break;
		if (!got_frame)
			return 0;
...
static int close_output_file(FFPlayer *ffp)
{
	VideoState *is = ffp->is;
...
static AVFrame* deep_copy_frame(AVFrame *frame)
{
...
static int video_thread(void *arg)
{
    ...
    for (;;) {
        ...
        while (ret >= 0) {
			...
			// liuyi add begin
			/*
			if (filter_frame = deep_copy_frame(frame)) {
				int got_frame;
				encode_write_frame(is, filter_frame, is->video_stream, &got_frame);
			}else {
				av_log(NULL, AV_LOG_ERROR, "Could not create get the copy frame \n");
			} */
			// liuyi add end
...
static int stream_component_open(VideoState *is, int stream_index)
{
    ...
    case AVMEDIA_TYPE_VIDEO:
		...
		open_output_file("C:\\Data\\record.mp4",is);// liuyi add
...
static int read_thread(void *arg)
{
	...
	// liuyi add  refer to http://ffmpeg.org/doxygen/3.4/transcoding_8c-example.html
	stream_ctx = av_mallocz_array(ic->nb_streams, sizeof(*stream_ctx));
	if (!stream_ctx)
		return AVERROR(ENOMEM);
    ...
    if (st_index[AVMEDIA_TYPE_VIDEO] >= 0) {
		...
		//AVCodecContext *codecContext = is->ic->streams[AVMEDIA_TYPE_VIDEO]->codec; // liuyi
		//InitFilter(codecContext);
    }
...
//      of the seek_pos/seek_rel variables
		...
		count++;
		if (count == 1000) {
				close_output_file(is);	// liuyi add
		}
    }
...
int main(int argc, char **argv) {
	...
	char **paras = (char **)malloc(sizeof(char*) * 4);
	paras[0] = "ffplay";
	paras[1] = "-vf";
	paras[2] = "drawtext=fontfile=arial.ttf:fontsize=20:text='hello world':x=100:y=100";
	paras[3] = "test.mp4";
	oldEntrence(4, paras);
	...
	const char *paras[] = { "ffplay",
		"-vf",
		"drawtext=fontfile=arial.ttf:fontsize=20:text='hello world':x=100:y=100",
		"test.mp4",
	};
