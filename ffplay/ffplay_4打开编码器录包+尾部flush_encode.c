...
typedef struct FFPlayer {
	...
	//AVOutputFormat *m_ofmt; // ffp->m_ofmt = ffp->m_ofmt_ctx->oformat;
...
int ffp_start_record(FFPlayer *ffp, const char *file_name)
{
	...
	for (int i = 0; i < is->ic->nb_streams; i++) {
		...
		/*将输入视频/音频的参数拷贝至输出视频/音频的AVCodecContext结构体
		 ...
		 avcodec_copy_context(out_stream->codec, in_stream->codec) < 0) 成功，已经弃用
		...
		*/
		...
		if (dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO ) {
			av_stream_set_r_frame_rate(out_stream, av_inv_q(dec_ctx->framerate));
		}
...
static int read_thread(void *arg)
{
	...
	//open_output_file("C:\\Data\\record.mp4",ffp);// liuyi add 
